...
impl IndexBuilder for SqlIndexBuilder {
    async fn build_index_if_needed(&self) -> Result<Option<Index>, IndexError> {
        ...
        info!(
            "CACHE_PERSISTENT_SIZE size found/default: {} records",
            cap_per
        );
        ...
        // do this in batches
        let nquery: i64 = query(
            "SELECT MAX(a.articleid)
                            FROM article as a
                            ",
        )
        .fetch_one(&pool)
        ...
        for batch in 0..num_batches {
            ...
            let mut infoboxes = infoboxes_q
                .await
                .unwrap()
                .into_iter()
                .map(|i| (i.get("articleid"), i.get("infoboxes")))
                .collect::<HashMap<i64, Vec<(String, String)>>>();
            let mut citations = citations_q
                .await
                .unwrap()
                .into_iter()
                .map(|c| (c.get("articleid"), c.get("citations")))
            ...
            batch_documents.into_iter().for_each(|d| {
                ...
                let new_document = Box::new(Document {
                    ...
                    infoboxes: infoboxes
                        ...
                        .collect(), //v.get("infoboxes").map(|v| v).unwrap_or_default(),
                    citations: citations
                        .unwrap_or_default()
                        .into_iter()
                        .map(|v| Citation { text: v })
                        .collect(), //v.try_get("citations").unwrap_or_default(),
                });

                if let Err(e) = pre_index.add_document(new_document) {
                    let title: String = d.get("title");
                    error!(
                        "Error in adding document with {{id: {}, title: {}}}:{}",
                        doc_id, title, e
                    );
                }
            });
            ...
            info!(
                "Building pre-index: {}% ({}s) - processed {} docs, cache size {}",
                (processed_docs as f32 / num_docs as f32) * 100.0,
                timer.elapsed().as_secs(),
                processed_docs,
                pre_index.cache_size()
            );
        }
