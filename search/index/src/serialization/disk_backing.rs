...
use utils::MemFootprintCalculator; // 1.3.1
...
static FREE_SPACE_BLOCKS: Lazy<Mutex<HashMap<u32, BTreeMap<u64, Vec<u64>>>>> =
    ...
    fn unload(&mut self) -> Result<(), Box<dyn Error>> {
        let offset = match self {
            Entry::Memory(v) => {
                ...
                Self::evict(FILE_HANDLES.lock().get_mut(&ID).unwrap(), prev)?
            }
