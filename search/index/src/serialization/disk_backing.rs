...
use std::{
    ...
    collections::{HashSet},
    path::{PathBuf, Path},
    env, fmt::{Display, Debug},
    fs::{File,remove_dir_all,create_dir,remove_file, rename},
    io::{Read, ErrorKind}, iter::Chain, marker::PhantomData,
    io,
    error::Error
};
...
V: Serializable + Debug
{
    ...
    fn evict_victim(&mut self) -> Result<(),Box<dyn Error>>
    {
        ...
        if self.online_map.len() > 0{
            ...
            let (k,v) = self.online_map.pop().ok_or(
                Box::new(IndexError { 
        ...
        Q: Debug + Equivalent<K> + Hash + Eq
    {
        ...
        let (removed_filename,remove_idx) = self.offline_file_name(k).ok_or(
            Box::new(IndexError{
                msg: format!("Tried to remove from offline map, but {:?} was not in the offline_map",k) ,
                kind: IndexErrorKind::LogicError
            }
        ))?;
        ...
        if !self.offline_set.swap_remove(k){ // note this swaps id's of last and this element
        ...
        Q: Hash + Eq + ToOwned<Owned = K>,
    {
        ...
        Ok(self.online_map.entry(key)
            .or_insert(v))
    }

    fn offline_file_name<Q : ?Sized >(&self, key: &Q) -> Option<(PathBuf,usize)> where 
        Q: Hash + Eq + Equivalent<K> + Debug,
    {
        self.offline_set.get_full(key)
...
impl <K,V, const R: u32> Drop for DiskHashMap<K,V,R> where 
K : Serializable + Debug + Hash + Eq + Clone,
V : Serializable
{
    fn drop(&mut self) {
