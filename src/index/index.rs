...
use std::{collections::HashMap,fmt,marker::{Send, Sync}};
...
impl Index for BasicIndex {
    fn get_links(&mut self,source: u32) -> Vec<u32>{
        match self.links.as_mut().expect_right("Index was not finalized.").get(&source){
            Some(v) => v.clone(),
            None => return Vec::new(),
        }
    ...
    fn finalize(&mut self){
        ...
        for (token,postings) in self.postings.iter(){
            ...
            self.doc_freq.insert(token.to_string(), unique_docs.len() as u32);
        }
...
impl BasicIndex{
    ...
    fn add_structure_elem(
        ...
        self.extent.entry(structure_elem.to_string())
            .or_insert(HashMap::new())
            .entry(doc_id)
            .or_insert(PosRange {
