...
impl BasicIndex{
    ...
    fn add_posting(&mut self, token: String, docid: u32, word_pos: u32){
        ...
        self.postings.entry(token.clone()).or_insert(Vec::<Posting>::new()).push(Posting{
            document_id: docid,
            position: word_pos,
        });

        let freq_map :  &mut HashMap<u32, u32> = self.term_freq.entry(token.clone()).or_insert(HashMap::new());
        *freq_map.entry(docid).or_insert(0) += 1;
        
    }

    pub fn add_document(&mut self, text: &str, doc_id: u32, categories: &str, article_links: &str, article_abstract: &str){
        let mut word_pos = 0;
        ...
        self.links.as_mut().expect_left("Index is not in buildable state").insert(doc_id,link_titles);
