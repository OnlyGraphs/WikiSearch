...
fn main() -> std::io::Result<()> {
    ...
    thread::spawn(move || {
        let mut retries = 3;
        while retries > 0{
            let status = run_grpc(index.clone(),grpc_address.clone(),connection_string.clone());
...
#[actix_web::main]
async fn run_grpc<'a>(index: Arc<RwLock<Box<dyn Index>>>, grpc_address: String, connection_string: String) -> std::io::Result<()> {
    ...
    let response = service.update_index(Request::new(CheckIndexRequest {}))
                    .await
                    .map_err(|c| Error::new(ErrorKind::Other, c))?;

    if !response.get_ref().success {
        return Err(Error::new(ErrorKind::Other, response.get_ref().err_code.to_string()));
    }
       
    info!("Built initial index.");
    // show it or error
    info!("{:?}",index.try_read().map_err(|c| Error::new(ErrorKind::Other, c.to_string())));
