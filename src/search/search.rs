...
pub fn execute_query(query: Box<Query>, index: &Box<dyn Index>) -> Vec<Posting> {
    ...
    match *query {
        ...
        Query::PhraseQuery { tks } => tks.iter().tuple_windows().fold(Vec::default(), |a,(prev,current)| {
                                    println!("TOKENS: {:?}", (prev,current));
                                    linear_merge(a,
                                        linear_merge( 
                                            index.as_ref().get_postings(prev).unwrap_or_default().to_vec(),
                                            index.as_ref().get_postings(current).unwrap_or_default().to_vec(), 
        ...
        Query::FreetextQuery { tokens } => tokens.iter().fold(Vec::default(), |a, t| {
            linear_merge(
                a,
                index.as_ref().get_postings(t).unwrap_or_default().to_vec(),MergeType::Union(MergeIdentity::FullMerge)
            )
        }),
    }
}
                    ...
                    match operation {
                        MergeType::Distance(dst) => {
                            if r.position.overflowing_sub(l.position).0 <= dst {
                                if l < r {out.push(*l); out.push(*r)} else  {out.push(*r); out.push(*l)}
                            } 
                        },
                        MergeType::Union(MergeIdentity::FullMerge) => if l < r {out.push(*l); out.push(*r)} else  {out.push(*r); out.push(*l)},
