...
use crate::index::{index::{BasicIndex,Index},index_structs::{Posting,PosRange}};
use crate::tests::test_utils::{get_document_with_text,get_document_with_links};
...
fn test_basic_index_get_postings() {
    ...
    idx.add_document(
        get_document_with_text(2,"d0",
            vec![("","aaa bbb")],
                "ccc ddd",
            vec!["eee fff"],
                "ggg hhh"));

    assert_eq!(*idx.get_postings("aaa").unwrap(),
                vec![Posting{
                        document_id: 2,
                        position: 0,
                    }]
    ...
    assert_eq!(*idx.get_postings("ddd").unwrap(),
            vec![Posting{
                    document_id: 2,
                    position: 3,
                }]
    ...
    assert_eq!(*idx.get_postings("ggg").unwrap(),
        vec![Posting{
                document_id: 2,
                position: 6,
            }]
...
fn test_basic_index_get_extent() {
    ...
    assert_eq!(*idx.get_extent_for("infobox", &2).unwrap(),
        PosRange{
            start_pos: 0,
            end_pos: 2,
        }
    ...
    assert_eq!(*idx.get_extent_for("infobox2", &2).unwrap(),
        PosRange{
            start_pos: 2,
            end_pos: 3,
        }
    ...
    assert_eq!(*idx.get_extent_for("citation", &2).unwrap(),
        PosRange{
            start_pos: 5,
            end_pos: 9}
    );
    
    assert_eq!(*idx.get_extent_for("categories", &2).unwrap(),
    PosRange{
        start_pos: 9,
        end_pos: 11}
);
