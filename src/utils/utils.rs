...
use std::mem::{size_of};
use std::hash::{Hash};
use crate::index::index_structs::{Posting, PosRange, DocumentMetaData};

pub trait MemFootprintCalculator{
    fn real_mem(&self) -> u64;
}

impl MemFootprintCalculator for String where 
{
    fn real_mem(&self) -> u64{
...
impl <T>MemFootprintCalculator for Vec<T> where 
    T: MemFootprintCalculator
{
    fn real_mem(&self) -> u64{
...
impl <K,V>MemFootprintCalculator for HashMap<K,V> where 
    K: MemFootprintCalculator,
    V: MemFootprintCalculator
{
    fn real_mem(&self) -> u64{
        self.iter().fold(0,|a,(k,v)| v.real_mem() + k.real_mem() + a) + size_of::<HashMap<K,V>>() as u64
    }
}

impl <K,V>MemFootprintCalculator for BiMap<K,V> where 
    K: MemFootprintCalculator + Eq + Hash,
    V: MemFootprintCalculator + Eq + Hash
{
    fn real_mem(&self) -> u64{
        self.iter().fold(0,|a,(k,v)| v.real_mem() + k.real_mem() + a) + size_of::<BiMap<K,V>>() as u64
    }
}


impl <L,R>MemFootprintCalculator for Either<L,R> where 
